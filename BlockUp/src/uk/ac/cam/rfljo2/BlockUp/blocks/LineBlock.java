...
public class LineBlock extends Block {
	...
	public void regenerateCells() {
		...
		if (getRotationState() == 0) {
			for (int i = 0; i < 4; i++) {
				Cell c = new Cell(getPivotPoint().getCol() - 2 + i,getPivotPoint().getRow(),cells[i].getType(),cells[i].getColour());
				cells[i] = c;
			}
		}
		...
		if (getRotationState() == 3) {
			for (int i = 0; i < 4; i++) {
				Cell c = new Cell(getPivotPoint().getCol(),getPivotPoint().getRow() - 2 + i,cells[i].getType(),cells[i].getColour());
				cells[i] = c;
			}
